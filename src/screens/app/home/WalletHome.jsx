...
const WalletHome = () => {
  ...
  const [walletBalance, setwalletBalance] = useState("Available Balance is INR 0");
  const logout = async () => {
  ...
  const getWalletBalance = async () => {
    ...
    if (response[0] == 200) {
      setwalletBalance("Available Balance is INR " + response[1].data.user.wallet.amount);
      setoverlay(false)
    }
    else {
  ...
  const navigateToScreen = async () => {
    if (amount == "" || amount == null) {
      ...
      navigation.navigate('WalletBankDetails', { totalAmount: totalAmount, enteredamount: amount, fromCurrency: fromCurrency, toCurrency: toCurrency, fees: fee, exchangeRate: exchangeRate })
    }
  }
          ...
          {walletBalance  &&
            <View style={styles.exchangeContainer}>
              <Text style={styles.exchangeText}>
                <Text style={styles.exchangeAmount}> {walletBalance} </Text>
              </Text>
            </View>
          }
